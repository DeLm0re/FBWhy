...
/*
@function
 * 			void ecrisSpriteBase(char *nom, int hauteur, int largeur, int idSprite)
 * 
 * @brief
 * 			Fonction qui ecrit sur l'ecran un sprite de base d'un personnage, à un endroit donné
 * 
 * @param
 * 			char *nom : Le nom du personnage
 * 			int hauteur : La position en hauteur où afficher le sprite
 * 			int largeur : La position en largeur où afficher le sprite
 * 			int idSprite : L'id du sprite à afficher
 * 
 * @return
 * 			void
...
void ecrisSpriteBase(char *nom, int hauteur, int largeur, int idSprite)
{
	...
	DonneesImageRGB *imageRGB = NULL;
	
	//Lecture de notre image sprite
	strcpy(buffer, CHEMIN_SPRITES);
	strcat(buffer, nom);
	strcat(buffer, ".bmp");
	imageRGB = lisBMPRGB(buffer); /*ALLOUR imageRGB
	
	// Affichage d'une image
	if (imageRGB != NULL) // Si l'image a pu etre lue
	{
		imagePixel *sprite = creerImagePixel(HAUTEUR_SPRITE, LARGEUR_SPRITE, 255); /*ALLOUE sprite
		
		imagePixel *image = conversionDonneesImageRGB(imageRGB);
		
		for(i = 0; i < HAUTEUR_SPRITE; i++)
		{
			for(j = (idSprite * LARGEUR_SPRITE); j < LARGEUR_SPRITE + (idSprite * LARGEUR_SPRITE); j++)
			{
				if( (image->pixels[i][j].rouge != 255) && (image->pixels[i][j].bleu != 0) && (image->pixels[i][j].vert != 255) )
				{
					sprite->pixels[i][j-(idSprite * LARGEUR_SPRITE)].rouge = image->pixels[i][j].rouge;
					sprite->pixels[i][j-(idSprite * LARGEUR_SPRITE)].vert = image->pixels[i][j].vert;
					sprite->pixels[i][j-(idSprite * LARGEUR_SPRITE)].bleu = image->pixels[i][j].bleu;
				}
			}
		}
		
		libereImagePixel(&image);
		
		DonneesImageRGB *spriteRGB = conversionImagePixel(sprite);
		
		// On affiche le sprite
		ecrisImage(hauteur, largeur, LARGEUR_SPRITE, HAUTEUR_SPRITE, spriteRGB->donneesRGB);
		
		libereDonneesImageRGB(&spriteRGB);
	}
}
*/
